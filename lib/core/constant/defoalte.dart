...
double arabickfontsize = 30.0;
double moshaffontsize = 20.0;
int bookmarkedAyah = 1;
int bookmarkedSura = 1;
bool fabIsClicked = true;
bool barck = false;
bool notif = false;
String dropdownValue = 'القاهرة';
...
Future saveSetting() async {
  ...
  await prefs.setInt("moshaffontsize", moshaffontsize.toInt());
  await prefs.setInt("arabickfontsize", arabickfontsize.toInt());
  await prefs.setBool("notif", notif);
  await prefs.setBool("barck", barck);
}

Future savecity() async {
  final SharedPreferences prefs = await SharedPreferences.getInstance();
  await prefs.setString("dropdownValue", dropdownValue);
}

Future getcity() async {
  try {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    dropdownValue = await prefs.getString("dropdownValue")!;
  } catch (_) {
    dropdownValue = 'القاهرة';
  }
}
...
Future getSettings() async {
  try {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    arabickfontsize = await prefs.getInt('arabickfontsize')!.toDouble();
    moshaffontsize = await prefs.getInt('moshaffontsize')!.toDouble();
    notif = await prefs.getBool('notif')!;
    barck = await prefs.getBool('barck')!;
  } catch (_) {
    arabickfontsize = 30.0;
    moshaffontsize = 20.0;
    barck = false;
    notif = false;
  }
